...
### 2026-06-28 — Backfill entries from real historical prices ("returns as of today")
- Problem: all tracker picks were seeded today so every return read ~0%. Added `TopPlaysTracker.backfill_entries(fetch_history, lookback_days=30)` + `POST /api/top-plays/backfill?lookback_days=30`. For each active pick it pulls real FMP EOD history (api_client.fmp_historical), sets entryDate/entryPrice to the real close ~30d ago, peakPrice to the real high over the window, lastPrice to latest close, and marks `backfilled:true`. Honest backtest — 100% real market prices.
- Ran on preview (44/44 updated). Hall of Fame now MAMA +41.6% peak (entry 2026-06-01 $14.25 → peak $20.18, 27d). Best Performers show real spread (MAMA +41.6%, NUVB +33.1%, IDYA +32.0% … NRGV peak +24.3% now -21.3%). Avg active now-return -1.5% (range -37.5%..+39.9%).
- TopPlays explainer updated with transparency line: current picks' entry seeded from real prices ~30d ago; new entrants tracked live from day one.
- Backfill is MANUAL (not auto) to avoid corrupting a genuine forward track record later. To apply on production: POST /api/top-plays/backfill once after the first universe build on Railway.
- Fixed a transient edit bug that merged the hall_of_fame def into a comment line (AttributeError) — corrected; ast-parse + live 200 confirmed.

### 2026-07-02 — Day Trades tab + scoring upgrades + quarterly growth fix (DONE, iteration_9.json 100% backend+frontend)
- **NEW "Day Trades" tab** (`/day-trades`, sidebar under AI Intelligence). Backend `day_trades_engine.py` (DayTradesEngine): sources FMP biggest-gainers + most-actives (filters out ETFs/leveraged, price $2-1500, gainers +3..75%), enriches each of ~16 candidates (7 FMP calls each: quote, 5-min bars, PT consensus, grades-consensus, grades, EOD history for 20d avg volume + StockNewsAPI). Computes VWAP, RSI14(5m), 9-EMA, opening-range H/L, relative volume; scores 0-100 (momentum, relVol, above-VWAP, RSI band, today's catalyst, analyst actions this week, Buy consensus). Trade plan per pick: entry zone (VWAP-hold pullback), Target1 +2%, Target2 +3%, stop (below VWAP, clamped -0.75%..-1.5%, R/R ≥1.3), 6-step "how to trade until sell" playbook (pre-trade, entry triggers, stop, scale-out at T1→breakeven, trail 9-EMA at T2, 15:45 ET time exit). Endpoints: `GET /api/day-trades/today` (15-min mongo cache `day_trades_cache`, `?refresh=true` to force; cold gen 60-120s), `GET /api/day-trades/scoreboard` (evaluates past picks vs real day high/low from EOD: target1/target2 hit, stopped, ambiguous→stopped conservative, closed_flat; win rate + avg realized; picks logged in `day_trades_log` keyed date+ticker). Frontend `DayTrades.jsx`: session-phase banner (ET), collapsible 5-step morning routine, risk disclaimer, 8 trade cards (catalyst w/ sentiment, technicals grid, 4-cell plan, R/R, analyst line, expandable playbook + signals + risk flags + analyst actions), scoreboard w/ history table. testids: day-trades-page, session-banner, morning-routine-toggle, day-trade-card-{T}, catalyst-{T}, trade-plan-{T}, expand-playbook-{T}, playbook-{T}, day-trades-refresh-btn, day-trades-scoreboard.
- **Short-term score upgrades** (company_universe.py `_short_term_score`): insiderActivity now weighted 0.10 (Net buying 85 / Mixed 55 / Net selling 40) — derived from pattern analysis: all top-5 peak-ROI winners had insider net buying. Thin-coverage haircut: no analystRating AND no analystPriceTarget → base×0.75 (demotes HKD/MAKO/ASBA-type unverified stories). Verified: top ranked names now skew Net buying (7/10).
- **Quarterly YoY growth fix**: enrich_company now fetches `income-statement?period=quarter&limit=6` (replaced annual financial-growth). revenueGrowth = latest-Q YoY, revenueAcceleration = latest-Q YoY minus prior-Q YoY, epsGrowth = latest-Q epsDiluted YoY. Fresh within ~3 months vs ~12. Verified: SNDK 10.4% (stale annual) → +251% (real latest quarter); AAPL sane 16.6%. Takes effect universe-wide on next scheduled rebuild (or POST universe build). Source labels updated ("FMP income-statement (latest quarter YoY)").
- **Pattern analysis delivered to user**: winners = healthcare revenue-inflection small/mid caps with insider buying + rising revisions; losers = thin-coverage/cyclicals with insider selling. 7 pattern-matched swing suggestions given with entry/target/stop (CRNX, VRDN, TSHA, SBSW, HNRG, CALX, IE).
- NOTE: preview URL for this fork is https://local-repair-fix.preview.emergentagent.com (old code-scan-repair URL detached mid-session and 404s).
